...
|---|---|---|
| Main/header/footer | 1,180px maximum (`--width-project-form`); 24px top (`space-6`), 40px horizontal (`space-10`), 24px below 1320 and 20px below 900; 34px Newsreader heading; 16px footer padding. Shell remains shared. | Confirm padding relative to AppLayout, persistent action placement and that keyboard focus is never hidden at 390/768/1440. The footer's existing draft-discard action is functional, not prototype decoration. |
...
| Visibility/rate options | Base-surface option rows with 12px padding/gap, 8px radius, input border, 16px radio and separate title/hint. Selected border/ring match type cards. | Native keyboard selection and disabled-fieldset behavior; selected project rate also needs the reference's adjacent compact amount instead of a full-width field. |
| Budget | 320px dropdown (`--width-form-select`), adjacent 160px total, 64px threshold, mono right-aligned values; scoped modes use task/person rows. | Align sizes/inline relationship; retain honest unavailable-mail state, exact values and real scoped controls. No hidden single total for scoped budgets. |
| Fixed fees | Segmented single/milestone/monthly choices; 200px fee, 200px monthly-day selector; milestone columns flexible name + 160px date + 140px amount + remove. | Compact controls and responsive row stacking still need alignment. Keep accessible labels and exact totals; describe explicit preparation rather than promise automatic invoicing. |
...
| Tasks | 16px-radius grouped card, serif 20px title/count, billable all/none, subtle info strip, compact task rows, real restriction action and add/suggestion footer. | Complete compact-row/rate/budget layout and empty/pending/error keyboard checks; do not copy the prototype's inert Everyone link. |
| Team | Matching grouped card; manager checkbox, avatar/identity/role, bill/cost information and removal; searchable 44px add control plus Add everyone. | Identity/role/avatar and compact rate/budget layout still differ. Preserve admin-only costs, explicit project overrides and real catalog search without fabricated sample rates. |
| Invoice defaults | 32px section gap; 240px payment terms and PO; 96px tax/discount; optional second-tax 160px name + 96px percentage with removal; serif 20px heading. | Compact widths/inline tax layout and complete keyboard flow remain. Custom terms and editable defaults already have persistence/consumer coverage; do not erase those controls to copy incomplete prototype behavior. |
