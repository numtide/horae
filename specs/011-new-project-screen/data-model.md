...
Generation payloads include the reviewed source/balance snapshot and exact source-level excess confirmations, not only client/period/defaults. Fee selections alter only invoice-owned descriptions, amounts and inclusion; time rows stay frozen to the current source read. The writer canonicalizes source order, rechecks current actor and financial values under the organization lock, and persists selected gross/net lines with the header in one transaction. Planned but unselected fees are not materialized. A new review gets a new mutation identity; uncertain retries preserve the prior complete payload. Settled and over-invoiced sources remain visible but initially unselected; editable initial controls require a fresh review after changes.

Before generation or draft editing is sent, the browser acknowledges a bounded (512 KiB) serialization of the exact request in sessionStorage, under a versioned organization/user key. Invoice routes check current authenticated manager authority before reading that key or exposing mutations. After a reload, an unresolved request offers explicit replay of the same payload, including the original request ID and excess confirmation; no new invoice or revision is created by an acknowledged replay. Success or a definite rejection clears the local record. Storage failures block sending or retain recovery as appropriate. This is per-tab recovery, not a durable invoice autosave: ordinary unsaved edits require discard confirmation on navigation, and closing the tab/clearing browser storage can remove its recovery record. Server authorization, revision and balance checks remain authoritative.
...
Project Detail reads the same fee preview through `get_project_fee_balances`. A read-only repeatable-read transaction validates current `project_read_access` billing/progress authority and scopes the calculation to one project in the same organization. `ProjectFeeBalance` contains the stable period key, description, currency and shared `InvoiceFeeBalance`, without unrelated invoice inputs or identities. Organization managers/admins may read it; member/project-lead calls are rejected rather than receiving hidden financial fields. Date filters retain invoice preparation semantics: monthly fees are confined to their own due-date range; single fees and milestones are included through the upper date. The browser defaults to the current UTC calendar month, preserves signed negative balances and hides stale rows while loading or after errors. No totals are transferred between monthly occurrences.

Preparation identifies time lines by entry UUID and fee lines by `(project_id, period_key)`, without needing a persisted occurrence ID. Fee context reports agreed amount, non-void contributions including drafts, and the signed balance before the proposed invoice. Each line's proposed net is absent until invoice defaults are resolved. Allocation uses source order (time UUID, then project UUID/period key) independently of date-based display order, sharing the checked helper used by persisted generation and draft edits. The editable initial form and generation writer use this same reviewed snapshot.
