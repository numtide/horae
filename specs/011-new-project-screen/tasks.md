...
- [ ] T025 [US3] Implement scoped/monthly/inclusion-aware consumption and unique logical alert records in `crates/horae/src/server_fns.rs`, `crates/horae/src/server_fns/projects.rs`, `crates/horae/src/scheduler.rs`; retain legacy plugin event behavior for legacy projects.
  - Backend checkpoint: `server_fns/budgets.rs` evaluates configured scopes/periods and atomically records unique alerts with pending outbox jobs; migration 0032 and the periodic recovery sweep are connected. T025 remains open until the authorized Projects progress projection and displayed remaining budgets use this evaluator. Mail delivery is separately pending under T027/T028.
