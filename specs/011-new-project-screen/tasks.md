...
- [ ] T036 [US4] Apply consistent authorization to project export count/stream and Harvest project/task/time-entry projections in `crates/horae/src/reports.rs`, `crates/horae/src/reports/limits.rs`, `crates/horae/src/reports/streaming.rs`, `crates/horae/src/harvest/mod.rs`.
  - Project checkpoint: CSV/XLSX reads and preflight limits plus Harvest project count/page/detail now share `project_read_access`. T036 remains open for Harvest task projections and complete time-entry privacy, including private budget metadata and administrator-only project costs.
