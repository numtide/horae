...
- [ ] T048 [US6] Align label grid/cards/footer/spacing and responsive states in `crates/horae/src/pages/new_project.rs`, `crates/horae/assets/css/horae.css`, `crates/horae/build.rs`; no inline styles, copied literals or utility duplication.
  - Type/visibility/rate cards and compact rate, total-budget, single/monthly-fee and invoice-default fields now match their mapped dimensions. The scrolling form is separate from the persistent actions, with scoped shell behavior and mapped gutters. Task/team rows use compact labelled overrides, scoped avatars, project roles and removal controls. Task entry has inline suggestions; teammate search uses the shared selector. Both suppress stale choices during loading and preserve input on failure with retry. Budget, monthly fee day and payment terms now also reuse SelectField without changing choices or custom terms. Milestones use compact four-column desktop rows and stack below 900px, with exact values/totals, labelled native date entry and removal focus return. Browser checks cover search, keyboard selection, duplicate protection, numeric typography, wrapping and forward Tab traversal; milestone geometry also checks 769/900/1180 around sidebar/layout transitions. This task is not complete: final heading/label typography, the task information strip and the full conditional keyboard/error-state pass remain required.
