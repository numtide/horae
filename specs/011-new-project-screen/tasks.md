...
- [ ] T023 [US3] Apply selected rate modes and separate billing/cost currencies to `crates/horae/src/server_fns/projects.rs`, `crates/horae/src/server_fns/reports.rs`, `crates/horae/src/server_fns/invoices.rs`, `crates/horae/src/harvest/mod.rs` and related report models/exports.
  - Cross-currency task enablement now rejects incompatible catalog inheritance and accepts explicit text amounts with their displayed currency, checking the current project currency and selected mode. Existing links remain idempotent and preserve zero/overrides. The original regression failed before the fix; 51 project/privacy/mutation tests pass. The current detail-screen caller still sends no explicit rate: connect a mode-conditioned, currency-labelled field and verify recovery before considering this path complete. Global catalog denomination and existing project-edit audits also remain open.
  - Hourly checkpoint: shared SQL resolver, all five operational queries, separate report cost currency, per-project cost overrides, mixed-invoice currency rejection and Harvest time-entry financial redaction are implemented. Imported configuration preservation is verified under T021. Existing task enablement now inherits a catalog rate only for configured task-billed time-and-materials projects, preserving legacy inheritance and existing links; its 36-case regression and all ten project-module tests pass. T023 remains open for the denomination/mutation audit, including global catalog defaults without currency metadata, explicit cross-currency task rates and existing project-edit paths. Fee schema is verified under T022 and downstream fee/export coverage under T026/T044.
