...
- [ ] T042 [US5] Implement project invoice preparation, mixed-default resolution, checked adjustments and draft-only override mutation in `crates/horae/src/server_fns/invoices.rs`.
  - Generation inherits or explicitly resolves defaults, limits selected projects, snapshots checked components and allows row-locked draft edits. The manager-authorized preparation API now returns read-only time/fee estimates and per-project defaults, including unresolved conflicts; it shares source queries and fee calendars with generation. Interactive integration remains required before completing this task. T043 owns editable controls; visible/exported component breakdowns are complete under T044.
