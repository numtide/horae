...
- [ ] T021 [US3] Add failing Rust/SQL parity and legacy/import rate fixtures in `crates/horae/src/server_fns/projects/tests.rs`, `crates/horae/src/server_fns/invoices/tests.rs` and `crates/horae/src/harvest/mod.rs` (FR-009–013, FR-017).
  - Import checkpoint: CSV reimport preserves legacy exact totals and zero/absent rates across spend, reports, invoices and Harvest list/detail. New configured project-task links reject incompatible source-rate currencies with per-row rollback and recovery. Complete configured-project settings/assignment/privacy preservation still needs its import fixture; T021 remains open.
