...
- [ ] T048 [US6] Align label grid/cards/footer/spacing and responsive states in `crates/horae/src/pages/new_project.rs`, `crates/horae/assets/css/horae.css`, `crates/horae/build.rs`; no inline styles, copied literals or utility duplication.
  - Type/visibility/rate cards and compact rate, total-budget, single/monthly-fee and invoice-default fields now match their mapped dimensions. Browser checks cover numeric typography, inline desktop budget placement, mobile rate wrapping and second-tax focus return at 390/768/1440; other-screen computed styles remain unchanged. This task is not complete: main/footer geometry, milestone/task/team rows, selector fidelity and the full keyboard/error-state pass remain required.
