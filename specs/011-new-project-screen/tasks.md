...
- [ ] T048 [US6] Align label grid/cards/footer/spacing and responsive states in `crates/horae/src/pages/new_project.rs`, `crates/horae/assets/css/horae.css`, `crates/horae/build.rs`; no inline styles, copied literals or utility duplication.
  - Type/visibility/rate cards and compact rate, total-budget, single/monthly-fee and invoice-default fields now match their mapped dimensions. The scrolling form is separate from the persistent actions, with scoped shell behavior and mapped gutters. Task/team rows now use compact labelled rate/budget overrides, scoped avatars, project roles and removal controls. Task entry uses the shared compact input group with inline suggestions; teammate search uses the shared selector. Both suppress stale choices during loading and retain input/selection on failure with an explicit retry. Browser checks cover search, keyboard selection, duplicate protection, numeric typography, inline desktop budget placement, mobile rate wrapping, second-tax focus return and forward Tab traversal at 390/768/1440. This task is not complete: heading/label typography, milestones, the task information strip, remaining selector fidelity and the full conditional keyboard/error-state pass remain required.
