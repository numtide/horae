...
(async () => {
  ...
  try {
    ...
    const clientEditor = await context.newPage();
    await clientEditor.goto(`${base}/clients`);
    const clientRow = clientEditor.getByRole('row').filter({ hasText: 'New project browser client' });
    await clientRow.getByRole('button', { name: 'Edit', exact: true }).click();
    await clientEditor.getByLabel('Name', { exact: true }).fill('Rejected currency rename');
    await clientEditor.getByLabel('Currency', { exact: true }).fill('USD');
    await clientEditor.getByLabel('Address (optional)', { exact: true }).fill('Retained address');
    await clientEditor.getByRole('button', { name: 'Save Changes', exact: true }).click();
    await expect(clientEditor.locator('.card .alert-danger')).toBeVisible();
    await expect(clientEditor.locator('.card .alert-danger')).toContainText('Cannot change currency while the client has a default rate');
    await expect(clientEditor.getByLabel('Name', { exact: true })).toHaveValue('Rejected currency rename');
    await expect(clientEditor.getByLabel('Currency', { exact: true })).toHaveValue('USD');
    await expect(clientEditor.getByLabel('Address (optional)', { exact: true })).toHaveValue('Retained address');
    await expect(clientRow).toContainText('EUR');
    await expect(clientRow).not.toContainText('Retained address');
    await clientEditor.getByLabel('Name', { exact: true }).fill('New project browser client');
    await clientEditor.getByLabel('Currency', { exact: true }).fill(' eur ');
    await clientEditor.getByRole('button', { name: 'Save Changes', exact: true }).click();
    await expect(clientEditor.getByRole('button', { name: 'Save Changes', exact: true })).toHaveCount(0);
    await clientEditor.reload();
    await expect(clientRow).toContainText('EUR');
    await expect(clientRow).toContainText('Retained address');
    await readsFinished(clientEditor);
    await clientEditor.close();
    console.log('PASS: client default-rate currency rejects relabelling, preserves failed input and permits compatible edits');
    ...
    const entrySaved = page.waitForResponse(response => response.url().includes('/api/create_time_entry') && response.status() === 200);
    await entry.getByRole('button', { name: 'Save entry', exact: true }).click();
    const savedEntry = await (await entrySaved).json();
    assert.equal(savedEntry.project_id, created);
    assert.equal(savedEntry.minutes, 60);
    const entryDate = savedEntry.spent_date;
    assert.match(entryDate, /^\d{4}-\d{2}-\d{2}$/);
    await expect(entry).not.toBeVisible();
    await readsFinished(page);
    await page.getByRole('link', { name: 'Reports', exact: true }).click();
    await page.locator('input[type="date"]').first().fill(entryDate);
    await page.locator('input[type="date"]').last().fill(entryDate);
    ...
    await page.getByLabel('Period from', { exact: true }).fill(entryDate);
    await page.getByLabel('Period to', { exact: true }).fill(entryDate);
    ...
    await page.route('**/api/save_invoice_draft*', route => route.abort());
    await page.getByRole('button', { name: 'Save invoice values', exact: true }).click();
    await expect(page.locator('.alert-danger')).toBeVisible();
    await expect(page.getByRole('button', { name: 'Retry invoice save', exact: true })).toBeEnabled();
    ...
    await expect(page.getByRole('button', { name: 'Mark Sent', exact: true })).toBeDisabled();
    await readsFinished(page);
    await page.unroute('**/api/save_invoice_draft*');
    const updated = page.waitForResponse(response => response.url().includes('/api/save_invoice_draft') && response.status() === 200);
    await page.getByRole('button', { name: 'Retry invoice save', exact: true }).click();
    const edited = (await (await updated).json()).invoice;
