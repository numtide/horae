...
(async () => {
  ...
  const selectEditor = async id => {
    ...
    await expect(page).toHaveURL(`${base}/projects/${id}/edit`);
  };
  ...
  const verifyEditor = async (id, mode, pendingRead = false) => {
    ...
    if (pendingRead) {
      ...
      await page.route('**/api/load_project_editor*', async route => { await blocked; await route.abort(); });
    }
    try {
      await openEditor(id);
      if (pendingRead) {
    ...
    if (pendingRead) {
      await expect(page.getByRole('alert')).toContainText('Could not load project');
      await expect(screen).toHaveCount(0);
      await readsFinished();
    ...
    const rate = screen.locator('#np-project-rate');
    if (mode === 'Project') {
      await expect(rate).toBeEnabled();
      await rate.fill('');
      await screen.getByRole('button', { name: 'Save changes', exact: true }).click();
      await expect(screen.getByRole('alert')).toContainText('Project rate is required');
      await rate.fill('0');
    } else {
    ...
    await screen.locator('#np-budget-mode').click();
    const budgets = page.getByRole('listbox', { name: 'Choose Budget', exact: true });
    await expect(budgets.getByRole('option', { name: 'Total project fees', exact: true })).toHaveCount(monetary ? 1 : 0);
    await budgets.getByRole('option', { name: 'Total project hours', exact: true }).click();
    await screen.locator('#np-budget-value').fill('3:30');
    const name = `Edited ${id}`;
    await screen.locator('#np-name').fill(name);
    if (process.env.HORAE_TEST_SCREENSHOT_DIR) await screen.screenshot({ path: `${process.env.HORAE_TEST_SCREENSHOT_DIR}/project-editor-${mode}-${page.viewportSize().width}.png` });
    let releaseSave;
    ...
    try {
      await screen.getByRole('button', { name: 'Save changes', exact: true }).click();
      await expect(screen.locator('#np-name')).toBeDisabled();
      await expect(screen.locator('#np-budget-value')).toBeDisabled();
    ...
    await expect(screen.locator('#np-name')).toBeDisabled();
    await expect(screen.locator('#np-name')).toHaveValue(name);
    await expect(screen.locator('#np-budget-value')).toHaveValue('3:30');
  ...
  try {
    ...
    for (const scope of ['task', 'person']) {
      ...
      await openEditor(id);
      await expect(page.locator('#np-budget-mode')).toBeEnabled();
      await expect(page.locator('#np-budget-mode')).toContainText(`Hours per ${scope}`);
      await expect(page.locator('#np-budget-value')).toHaveCount(0);
      await page.locator('#np-name').fill(`Scoped ${scope}`);
    ...
    for (const id of ['np-name', 'np-project-rate', 'np-budget-mode']) await expect(page.locator(`#${id}`)).toBeEnabled();
    await expect(page.locator('#np-currency')).toBeDisabled();
    await expect(page.locator('#np-project-type')).toBeDisabled();
    await expect(page.getByText('Existing rate hierarchy', { exact: true })).toBeVisible();
    ...
    await page.route('**/api/load_project_editor*', async route => { await switching; await route.continue(); });
    try {
      await selectEditor(firstConfiguredId);
      await expect(page.getByText('Loading project…', { exact: true })).toBeVisible();
      await expect(page.locator('#np-project-type')).toHaveCount(0);
      await expect(page.getByRole('button', { name: 'Save changes', exact: true })).toHaveCount(0);
    } finally { releaseSwitch(); }
    await expect(page.getByRole('radio', { name: /^Task hourly rate/ })).toBeChecked();
    await expect(page.locator('#np-currency')).toBeDisabled();
    await readsFinished();
    await page.unroute('**/api/load_project_editor*');
    await page.locator('.np-footer').getByRole('button', { name: 'Cancel', exact: true }).click();
