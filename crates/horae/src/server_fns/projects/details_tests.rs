...
#[serial]
async fn project_edit_policy_uses_current_role_and_configuration(pool: PgPool) {
    for project_type in [
        ...
        for (mode, scope) in [
            ...
            for role in [OrgRole::Admin, OrgRole::Manager, OrgRole::Member] {
                ...
                .unwrap();
                let policy =
                    fetch_project_edit_policy(&pool, ids.org_id, ids.user_id, ids.project_id)
                        .await
                        .unwrap();
                ...
                assert_eq!(
                    (
                        policy.project_id,
                        policy.configured,
                        policy.rate_editable,
                        policy.budget_editable,
                        policy.monetary_budget_allowed
                    ),
                    (
                        ids.project_id,
                        mode.is_some(),
                        mode.is_none()
                            || (project_type == ProjectType::TimeAndMaterials
                                && mode == Some("project")),
                        mode.is_none() || scope == "project",
                        mode.is_none() || project_type == ProjectType::TimeAndMaterials
                    )
                );
            }
            sqlx::query!(
    ...
    for (org, user, project) in [
        ...
        assert!(
            fetch_project_edit_policy(&pool, org, user, project)
                .await
                .unwrap()
                .is_none()
        );
